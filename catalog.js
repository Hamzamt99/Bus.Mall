...
function updateCounter() {
  let headerCounter = document.getElementById('itemCount');
...
function updateCartPreview() {
  let cartItems = document.getElementById('cartContents');
  // TODO: Get the item and quantity from the form
  let selectedItem = event.target.items.value;
  let selectedQuantity = event.target.quantity.value;
  // TODO: Add a new element to the cartContents div with that information
  let pEl = document.createElement('p');
  pEl.textContent = `${selectedQuantity} : ${selectedItem}`
  cartItems.appendChild(pEl);
}
